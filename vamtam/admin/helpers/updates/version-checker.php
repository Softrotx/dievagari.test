...
class Version_Checker {
	...
	public function check_license( $args ) {
		...
		if ( ! empty( $key ) && VamtamFramework::license() !== self::$VALID_LICENSE ) {
			...
			if ( ! is_wp_error( $raw_response ) ) {
				if ( $raw_response['response']['code'] >= 200 && $raw_response['response']['code'] < 300 ) {
					if ( ! $no_ajax ) {
						...
						esc_html_e( 'Valid Purchase Key.', 'mozo' );
						echo '<p>';
						esc_html_e( 'Congratulations! You have succesfully registered your product.', 'mozo' );
						echo '</p>';
						echo '</span>';
					}
					...
					if ( ! $no_ajax ) {
						echo '<span id="fail">';
						esc_html_e( 'Incorrect Purchase Key.', 'mozo' );
						echo '<p>';
						esc_html_e( 'Please check your purchase code and re-enter.', 'mozo' );
						echo '</p>';
						echo '</span>';
					}
					VamtamFramework::license( self::$INVALID_LICENSE );
				...
				if ( ! $no_ajax ) {
					...
					esc_html_e( 'Cannot validate Purchase Key. Please try again later. If the problem persists your server might not have the curl PHP extension enabled.', 'mozo' );
					echo '</span>';
				}
			}
